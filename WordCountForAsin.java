...
public class WordCountForAsin extends Configured implements Tool{
	...
	public static class WordCountForAsinMapper extends
		...
		public void map(LongWritable key, Text value, Context context)
			...
			context.write(new Text(wordAndAsin[1]), new Text(wordAndAsin[0] + "=" + wordAndAsinCounter[1])) ;
		...
		int sumofWordsInAsin = 0 ;
		 Map<String, Integer> tempCounter = new HashMap<String, Integer>();
	        for (Text val : values) {
	            ...
	            sumofWordsInAsin += Integer.parseInt(val.toString().split("=")[1]);
	        }
	...
	public int run(String[] args) throws Exception {
		...
		FileInputFormat.addInputPath(wordCountForAsin, new Path(Tfidf.INPUTPATH2));
		FileOutputFormat.setOutputPath(wordCountForAsin, new Path(Tfidf.OUTPUTPATH2));
